...
public class LayerHeldItemGuard implements LayerRenderer<EntityGuardIllager> {
    ...
    public void doRenderLayer(EntityGuardIllager entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        ...
        if (abstractillager$illagerarmpose != AbstractIllager.IllagerArmPose.ATTACKING) {
            if (!itemstack.isEmpty()) {
                ...
                if (Block.getBlockFromItem(item).getDefaultState().getRenderType() == EnumBlockRenderType.ENTITYBLOCK_ANIMATED) {
                    GlStateManager.translate(0.0F, -0.2875F, -0.46F);
                    ...
                    GlStateManager.translate(0.0F, -0.2875F, -0.46F);
                    float f3 = 0.875F;
                    GlStateManager.scale(0.875F, 0.875F, 0.95F);
